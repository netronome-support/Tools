...
integer_str (char *str, int value)
{
    sprintf(str, "%d", value);
    return str;
}
...
static inline const char *
ipaddr_str (char *str, ipv4_addr_t ipaddr)
{
    ipv4_addr_t n = htonl(ipaddr);
    inet_ntop(AF_INET, &n, str, INET_ADDRSTRLEN);
    return str;
}

static inline const char *
ipaddr_nr_str (ipv4_addr_t ipaddr)
{
...
static inline const char *
hwaddr_str (char *str, const eth_addr_t hwaddr)
{
